...
breaks reproducibility (§3 NFR: "reproducible predictions").
"""
...
def build_technical_features_for_universe(lake: Lake) -> pd.DataFrame:
    """Compute the technical block for every symbol with silver price data,
    add cross-sectional rank transforms, and return the combined feature
    matrix (not yet written to the lake -- see persist_features)."""
    prices = lake.read_all(DataLayer.SILVER, "prices")
    if prices.empty:
        return pd.DataFrame()

    per_symbol = [technical.compute_technical_features(group) for _, group in prices.groupby("symbol")]
    matrix = pd.concat(per_symbol, ignore_index=True)

    matrix = add_cross_sectional_rank(matrix, TECHNICAL_FEATURE_COLUMNS)
