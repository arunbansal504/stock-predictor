...
| **Expected return amount (₹)** | Per stock: allocated amount × that stock's expected return. At the portfolio level: only the *actually-allocated* rupees (investment amount × capital allocated %) × the portfolio's expected return — money sitting unallocated because of a diversification shortfall (see **Capital allocated**) is treated as flat, not silently credited with the same growth as the invested portion. Same caveat as **Expected return** above — a historically-derived rupee figure, not a forecast. |
| **Expected final value (₹)** | Investment amount + expected return amount (portfolio level, as defined just above) — what your total money is estimated to become if history repeats itself over this horizon, with any unallocated portion left unchanged. Still just a historical average dressed in rupees, not a promise. |
